...
{
    // State Management
    public enum ElementState { ACTIVE, MOVE, WAIT, ROTATE, DESTROY };
    ...
    protected int playersActive = 0; 
    public int GetPlayersActive() { return playersActive; }
    ...
    private RotationInteraction rotInt;
    private MoveInteraction movInt;
    ...
    private Player FindRotatingPlayer()
    {
        if (playersActive == 1)
        {
            for (int i = 0; i < playerArray.Length; i++)
        ...
        Debug.LogWarning("FindRotatingPlayer Error.");
        return null;
    }
    ...
    public void ChangeState(ElementState state)
    {
        ...
        else if (state == ElementState.DESTROY)
        {
            frame.color = errorColor;
            waitCircle.color = errorColor;
            icon.color = errorColor;
            icon.sprite = iconDestroy;
    ...
    public bool PlayerIsConnected(Player player)
    {
        for (int i = 0; i < playerArray.Length; i++)
        {
