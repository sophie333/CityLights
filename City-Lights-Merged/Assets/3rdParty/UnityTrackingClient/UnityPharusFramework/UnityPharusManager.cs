...
namespace UnityPharus
{
	...
	public class UnityPharusManager : MonoBehaviour, ITrackingManager
    {
        ...
        public class PharusSettings
        {
			...
			[Tooltip("in pixel")]
			[SerializeField] private float _targetFloorWidth = 16;
			[Tooltip("in pixel")]
			[SerializeField] private float _targetFloorHeight = 9;
            ...
            public float TargetFloorWidth
            {
                get { return this._targetFloorWidth; }
                set { this._targetFloorWidth = value; }
            }
            public float TargetFloorHeight
            {
                get { return this._targetFloorHeight; }
                set { this._targetFloorHeight = value; }
			}
			public float StageX
			{
...
//			Debug.Log ("UnityPharusManager config loaded, continue InitInstance");
            if (m_unityPharusXMLConfig != null)
			{
				...
				{
					m_pharusSettings.TargetFloorWidth = configResolutionIntX;
					m_pharusSettings.TargetFloorHeight = configResolutionIntY;
					//Debug.Log(string.Format("XML config: new target resolution: {0}x{1}", m_pharusSettings.TargetFloorWidth, m_pharusSettings.TargetFloorHeight));
				}
				else
				{
					Debug.LogWarning(string.Format("XML config: invalid resolution config, using resolution specified in PharusManager prefab instead: {0}x{1}", m_pharusSettings.TargetFloorWidth, m_pharusSettings.TargetFloorHeight));
				}
        ...
        private void UpdateDebugGUI()
		{
			...
			if (resolutionStatus != null) {
				resolutionStatus.text = string.Format ("Resolution: {0} x {1}", m_pharusSettings.TargetFloorWidth, m_pharusSettings.TargetFloorHeight);
			}
        }
...
//				Debug.Log ("no errors occured during config file load");
		...
		public float TargetScreenWidth
		{
			get { return Instance.m_pharusSettings.TargetFloorWidth; }
		}
		public float TargetScreenHeight
		{
			get { return Instance.m_pharusSettings.TargetFloorHeight; }
		}
        ...
        {
           // return new Vector2(x * m_pharusSettings.TargetFloorWidth, m_pharusSettings.TargetFloorHeight - y * m_pharusSettings.TargetFloorHeight);
            Vector2 newPos = new Vector2(x * m_pharusSettings.TargetFloorWidth - m_pharusSettings.TargetFloorWidth / 2,
    m_pharusSettings.TargetFloorHeight / 2 - y * m_pharusSettings.TargetFloorHeight);
            return newPos;
        }
		#endregion
